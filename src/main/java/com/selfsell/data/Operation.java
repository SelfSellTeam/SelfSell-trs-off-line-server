...
import static com.selfsell.data.Operation.OperationType.CALL_CONTRACT_OP_TYPE;
import static com.selfsell.data.Operation.OperationType.DEPOSIT_OP_TYPE;
import static com.selfsell.data.Operation.OperationType.IMESSAGE_MEMO_OP_TYPE;
import static com.selfsell.data.Operation.OperationType.TRANSFER_CONTRACT_OP_TYPE;
import static com.selfsell.data.Operation.OperationType.WITHDRAW_OP_TYPE;
...
public class Operation {
  ...
  public static Operation createWithdraw(SSCPrivateKey sscPrivateKey, long amount) {
    Operation operation = new Operation();
    operation.setOperationType(WITHDRAW_OP_TYPE);
    byte[] balanceId = new WithdrawCondition(sscPrivateKey.getAddress()).getBalanceId();
    operation.setData(
        MyByte.builder()
              .copy(balanceId)
              .copy(amount)
    ...
    operation.setJson(
        JSON.build()
            ...
            .add("data", JSON.build()
                             ...
                             .add("amount", amount)
                             .add("balance_id", new SSCAddress(balanceId, SSCAddress.Type.BALANCE_ID).getAddressStrStartWithSymbol())
                             .get())
            .get());
    return operation;
  }

  public static Operation createDeposit(SSCAddress address, long amount) {
    Operation operation = new Operation();
  ...
  public static Operation createCallContract(
    ...
    balances.put(new WithdrawCondition(sscPrivateKey.getAddress()).getBalanceId(),
                 costLimit.getAmount() + transactionFee.getAmount());
    Operation operation = new Operation();
    operation.setOperationType(CALL_CONTRACT_OP_TYPE);
    operation.setData(
        MyByte.builder()
              .copy(sscPrivateKey.getPublicKey(true))
              .copy(balances)
              .copy(contract.getSscAddress().getEncoded())
    ...
    operation.setJson(
        JSON.build()
            ...
            .add("data", JSON.build()
                             .add("caller", new SSCAddress(sscPrivateKey.getPublicKey(true), SSCAddress.Type.PUBLIC_KEY)
                                 .getAddressStrStartWithSymbol())
                             .add("balances", mapFlatToList(balances))
                             .add("contract", contract.getSscAddress().getAddressStrStartWithSymbol())
  ...
  public static Operation createCallContract(
      ...
      String args,
      Asset costLimit) {
    SSCAddress sscAddress = new SSCAddress(contractId, SSCAddress.Type.CONTRACT);
    Map<byte[], Long> balances = new HashMap<>();
    Asset transactionFee = new Asset(Transaction.requiredFees);
    balances.put(new WithdrawCondition(sscPrivateKey.getAddress()).getBalanceId(),
                 costLimit.getAmount() + transactionFee.getAmount());
        ...
        JSON.build()
            ...
            .add("data", JSON.build()
                             .add("caller", new SSCAddress(sscPrivateKey.getPublicKey(true), SSCAddress.Type.PUBLIC_KEY)
                                 .getAddressStrStartWithSymbol())
                             .add("balances", mapFlatToList(balances))
                             .add("contract", sscAddress.getAddressStrStartWithSymbol())
                             .add("costlimit", costLimit.toJSON())
                             .add("transaction_fee", transactionFee.toJSON())
                             .add("method", method)
                             .add("args", args)
  ...
  public static Operation createContractTransaction(SSCPrivateKey sscPrivateKey, String contractId, Asset costLimit, Asset amount){
    SSCAddress sscAddress = new SSCAddress(contractId, SSCAddress.Type.CONTRACT);
    Map<byte[], Long> balances = new HashMap<>();
    Asset transactionFee = new Asset(Transaction.requiredFees);
    balances.put(new WithdrawCondition(sscPrivateKey.getAddress()).getBalanceId(),
    ...
    operation.setJson(
            JSON.build()
                    ...
                    .add("data", JSON.build()
                            .add("from", new SSCAddress(sscPrivateKey.getPublicKey(true), SSCAddress.Type.PUBLIC_KEY)
                                    .getAddressStrStartWithSymbol())
                            .add("costlimit", costLimit.toJSON())
                            .add("transaction_fee", transactionFee.toJSON())
                            .add("transfer_amount", amount.toJSON())
                            .add("balances", mapFlatToList(balances))
                            .add("contract_id", sscAddress.getAddressStrStartWithSymbol())
                            .get())
                    .get());
    return operation;
  }
  ...
  private static List<String[]> mapFlatToList(Map<byte[], Long> balances) {
    ...
    balances.forEach((k, v) -> result.add(
        new String[]{
            new SSCAddress(k, SSCAddress.Type.BALANCE_ID).getAddressStrStartWithSymbol(),
            v.toString()
        }));
    return result;
  }
